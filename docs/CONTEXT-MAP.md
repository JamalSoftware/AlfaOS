...
## Field Notification Foundation — NF-1 ENTREGUE, NF-2 em diante PLANNED
...
**O lado do SERVIDOR está pronto (`NF-1`); nenhum push chega a um aparelho ainda.** O Flutter não obtém token — `PushRegistrationService` continua sendo o `Noop`, sem `firebase_messaging`, sem permissão de Android e sem deep link. Isso é `NF-2` a `NF-5`. Código novo: `src/lib/push/fcm.ts` (provider e mapper de erro) e `src/lib/push/bootstrap.ts` (a escolha, uma vez por processo).
...
* **Falta**: o lado Flutter (`firebase_messaging`, permissão, obtenção e rotação do token) e o deep link do toque.
...
**Quando NÃO:** a central de notificações in-app (já existe e funciona sem push), notificação do painel web (outro assunto), ou qualquer coisa que não atravesse o provider. **`NF-1` está entregue; `NF-2` a `NF-7` não foram iniciadas.**
