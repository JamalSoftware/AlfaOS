...
**O controle de camadas mora FORA do canvas, e isso inverteu a decisão da `CTO-3.2.2`.** Dentro, ele cobria os botões `+`/`−` do Leaflet. Fora, custa altura — e a conta foi paga na faixa do mapa, hoje `320/360/380/400` (era `380/440/500/560`). Em 1440×900 o documento fecha em 900px exatos. **A regra que não mudou:** altura de mapa em pixels, nunca fração de tela.
...
**As iniciais do cliente podem entrar num `divIcon` por causa da GARANTIA DE SAÍDA.** A regra do projeto continua: nome digitado por gente não entra em HTML cru. `customerInitials` devolve `[A-Z]{0,2}`, com teste de entrada hostil — e é o contrato de saída, não a boa vontade do cadastro, que autoriza. Nome completo só no popup.
