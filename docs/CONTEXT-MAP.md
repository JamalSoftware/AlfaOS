...
**Carregar também:** `docs/MASTER-PLAN.md` — a sequência até o lançamento e as fatias V1, cada uma com dependências, entregas, testes, segurança e risco. Concluídos e congelados: Mapa Operacional V1, `DASH-1` + `DASH-1a`, `HOTFIX-FIELD-01`, `TL-1`, `EV-1` e `GS-1`. **`CORE FUNCTIONAL V1 — FEATURE COMPLETE`** (13/09/2026): as fatias funcionais da primeira versão estão feitas, e isso **não** é production ready. **Fase atual: `RC-1` — Release Candidate / Hardening:** `RC-1A` (auditoria, `OWNER DECISION REQUIRED`), `RC-1B` (segurança, configuração, tenancy e isolamento de teste, `APPROVED` / `CLOSED` — `docs/SECURITY.md` §8.21) e `RC-1C` (o contrato de localização do cliente, `APPROVED` / `CLOSED` em 15/09/2026 com a validação física do dono, depois das três correções da captura de GPS — contrato final congelado em `docs/TECHNICIAN-EXECUTION.md` §13.8; `docs/SECURITY.md` §8.22 e §8.22.1); a `RC-1D` está `APPROVED` / `CLOSED` e o `DIAG-AUTO-1` `CODE APPROVED` / `CLOSED` (16/09/2026), com o agendador de produção `PENDING RC-1F`; os débitos restantes estão no §12 dele. Ele **não é um segundo PRD**: onde os dois divergirem, o PRD vence.
...
* **Auditoria de storage é só leitura** (`npm run storage:audit`). `--resanitize-legacy` e `--purge-orphans` exigem `--apply` e **ordem do dono** — nunca rodar contra storage real por iniciativa própria. A re-sanitização não sobrescreve a original. **O expurgo exige escopo, validado na função:** `--apply` sem escopo é recusado; `--scope missing-company` é o ÚNICO que apaga; `--scope active-company` é recusado até decisão de retenção do dono; não existe escopo "todos" — não criar `--all` nem equivalente, que devolveria a mistura dos grupos.
...
* **Confirmar exige GPS e vale até `LOCATION_CONFIRM_MAX_DISTANCE_M` (100 m, inclusivo)**, com a distância calculada no servidor por `distanceInMeters` e comparada no metro inteiro. A regra vem **antes** da escrita; acima dela, nada é gravado. Distância no corpo é recusada. O aplicativo lê o limite do pacote (`location.confirmMaxDistanceMeters`) — não o copie para uma constante no Dart.
* **Coordenada de cliente só pelo GPS do aparelho**: `MANUAL` e meia coordenada são recusados na correção. Sem GPS, só endereço. Só as duas rotas do Field escrevem localização (há teste estrutural); `ADMIN` e `DISPATCHER` não têm porta de escrita.
* **A confirmação não muda de assinatura na trilha** (mesmo ponto, de não verificado para verificado) — é por ela que a timeline e o pacote técnico a reconhecem. Os números vão no `metadata` do evento `LOCATION_CONFIRMED`, e a leitura da OS (`getCompanyServiceOrder`) **remove a posição do aparelho** da saída.
* **Quem mostra localização lê a autoridade**: o mapa, o cartão "Localização do cliente" (`src/lib/customer-location-card.ts`, `ADMIN`, somente leitura) e a timeline. A projeção de `Customer` nunca alimenta tela nova. A formatação da distância é uma só (`formatDistanceMeters`, `src/lib/customer-location-presentation.ts`), e a do Dart repete a mesma conta com os mesmos vetores de teste.
* **RC-1C-HOTFIX — precisão ≤ `LOCATION_GPS_MAX_ACCURACY_M` (50 m) para confirmar e para corrigir com coordenada**, sobre o valor bruto, exigida no servidor (`requireGpsAccuracy`) **antes** da distância; ausente ou zero é recusa. Viaja no pacote (`location.gpsMaxAccuracyMeters`) — o Dart usa o do pacote e cai para 50 sem ele, nunca para "sem limite".
* **RC-1C-HOTFIX — no Field, Confirmar e Corrigir só usam posição da CAPTURA** (`apps/field/lib/core/location/operational_position.dart`): fluxo novo de leituras, cada uma julgada por coordenada, idade (≤ 10 s pelo instante da PRÓPRIA leitura — desde a RC-1C-HOTFIX-2 ela pode ter nascido antes de a captura abrir; exigir o "depois" recusava a posição que o provedor fundido entrega com o aparelho parado, e nenhuma outra vinha) e precisão — a **medida** (`measuredAccuracyMeters`), nunca só `Position.hasAccuracy`, que o `geolocator_android` 4.6.2 perde e chega `false` em toda leitura do Android (RC-1C-HOTFIX-3; a borda com o plugin é testada em `apps/field/test/operational_position_platform_test.dart`); a primeira aceitável encerra e desliga o GPS; sem ela em ~20 s, falha — a leitura ruim nunca vira posição. **Nunca `getLastKnownPosition`, nunca coordenada em log** (testes estruturais). `getCurrentPosition` não serve para isto: responde com a primeira posição que o sistema entregar, que é a APROXIMADA de 2000 m quando só ela foi concedida. O check-in e o ponto continuam com `LocationService` (informam, não bloqueiam).
* **RC-1C-HOTFIX — correção com GPS escolhido e sem posição válida é recusada**, no controlador e no servidor; não vira correção só de endereço.
...
**`/minhas-os` — "Hoje" é o dia civil da EMPRESA** (`HOTFIX-FIELD-01` `APPROVED`): `Company.timezone` → `resolveTimezone` → `civilDayBoundsIn`, intervalo `[início, início do dia seguinte)`. O defeito antigo — `new Date(ano, mês, dia)`, o fuso do processo — está resolvido; contrato em `docs/TECHNICIAN-EXECUTION.md` §9. **Débito RESOLVIDO na `RC-1D`:** "Próximas" juntava futuro, sem data e vencido. A fila passou a ter cinco seções exclusivas — Em atendimento · **Atrasadas** · Hoje · Próximas · **Sem agendamento** —, com "Hoje" na frente de "Atrasadas" (o contrato do `HOTFIX-FIELD-01` continua intacto) e a regra de atrasada vinda do painel (`isOverdueServiceOrder`, ao lado de `serviceOrderSliceWhere("atrasadas")`; PRD §380). Contrato em `docs/TECHNICIAN-EXECUTION.md` §9.
...
**Código interno nunca vai para a tela — `RC-1D`.** A timeline da OS traduz por `serviceOrderEventLabel` (`src/lib/service-order-event-labels.ts`); código sem rótulo vira "Evento registrado", e o código cru fica no `title`, como a "Atividade recente" do painel já fazia. Um teste varre o fonte atrás de todo `event: "..."` gravado e exige rótulo — **quem escrever um evento novo sem rótulo descobre ali**. A mesma regra fechou os outros códigos que apareciam crus (provedor de ERP, perfil de acesso, plataforma do aparelho, tipo de conexão) e a copy que prometia funcionalidade "nas próximas versões". Acessibilidade das telas operacionais tem auditoria própria em `e2e/accessibility.spec.ts` (nome acessível em todo controle, rótulo em todo campo, foco visível).
...
* **O diagnóstico atual não sustenta tempo real** (§337): refresh é sob demanda com gatilho na OS, e o teto é **10 por minuto, por usuário dentro da empresa** — uma CTO de 8 portas consumiria 8 das 10 de quem a abriu. Por isso a CTO mostra o último estado conhecido **com a idade da leitura**.

**Verificação automática de conectividade — `DIAG-AUTO-1`** (`docs/CTO-NETWORK-DISTRIBUTION.md` §48; PRD §370 e §390; `docs/SECURITY.md` §8.24). `npm run diagnostics:refresh` reconfere os clientes com conexão física ATIVA cuja última verificação venceu (~5 min). **É um comando, não um daemon** — o repositório não tem agendador, e o padrão é o de `scripts/outbox-worker.ts`; quem agenda é o operador, e sem isso nada se atualiza sozinho (a tela avisa com "Leitura desatualizada"). **Duas colunas novas em `CustomerDiagnosticSnapshot`:** `statusSince` (desde quando o estado atual começou — NUNCA derivar duração de `observedAt`, que é reescrito a cada verificação) e `refreshLeaseUntil` (a reserva que impede dois ciclos de chamarem o mesmo cliente). **Não existe `STALE`**, não existe segunda autoridade de estado, e falha de provider continua sem escrever nada. O ciclo não passa pelo teto de 10/min, que é de usuário. **Regras da `RC-1F-A` (`docs/CTO-NETWORK-DISTRIBUTION.md` §48.10), não desfazer:** a seleção NÃO é a autoridade final — com a reserva na mão, o ciclo relê o banco e descarta quem outro ciclo acabou de verificar (`skippedFresh`); a lista é ordenada por justiça (com leitura: tentada há mais tempo primeiro, pelo carimbo de `refreshLeaseUntil`; sem leitura: ordem estável por id GIRADA por tick, `rotacaoDosNuncaVerificados` — nunca sorteio) e nunca pela ordem física; o teto conta verificações que CHEGAM ao provider; o agendador NÃO nomeia provider nem `externalId` (teste de fonte); e **o prazo é contrato do provider**: `fetchCustomerConnectivity(ref, context)` recebe o `AbortSignal` de `runWithDiagnosticDeadline`, e todo adapter que faça I/O o repassa a cada requisição (§48.11). Cadência PLANEJADA: tick de 1 min, alvo de 5, aviso aos 10; nenhum agendador ativo. Configuração: concorrência 1..12 (padrão 6), teto 1..1000 (300), lote do outbox 1..500 (50); fora da faixa o comando sai com 2 nomeando a variável. **Um cliente só:** `npm run diagnostics:refresh -- --customer-id <id interno>` é a ferramenta oficial de validação e suporte (`docs/MASTER-PLAN.md` §17.2) — estreita a seleção em SQL (`findConnectionsDueForCheck(..., customerId)`) e mais nada; **não é force refresh** (recente, reservado ou desligado não é consultado, e nenhum outro cliente entra no lugar), não nomeia provider, e flag malformada sai com 2 em vez de virar "sem filtro". Não criar `--force`, `--receitanet-id`, `--sgp-id` nem busca por nome.

**Observabilidade dos clientes na tela da CTO — `RC-1D`** (`docs/CTO-NETWORK-DISTRIBUTION.md` §47; PRD §372). `/ctos/[id]` mostra o resumo de clientes (ativos · online · offline · sem leitura · OS abertas) e, por porta ocupada, cadastro, conectividade, idade da leitura e OS abertas. **Nenhuma autoridade nova:** `src/lib/cto-client-connectivity.ts` só compõe `getCtoOperationalSummaries` e `getCtoPortCustomers` — as MESMAS funções do popup —, então detalhe e popup não divergem (teste compara número a número). Filtros locais em duas unidades (`src/lib/cto-port-filters.ts`): porta conta posição, cliente conta cliente ativo; porta livre nunca diz "Sem leitura". Só leitura: zero chamada a provider, zero escrita, consultas constantes, `ADMIN` + capability como já era. **Não criar `CTOPort.connectivityStatus`, `Customer.online` nem contagem persistida na CTO** — as contagens são derivadas (PRD §372).
