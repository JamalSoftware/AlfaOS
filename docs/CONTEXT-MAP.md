...
**A `CTO-2.7` é o checkpoint do `CTO-2`, e AGUARDA validação do dono.** Fase de validação, com **zero diff de produção**: onze dos quinze critérios de aceite pertencem ao `CTO-2` e estão rastreados (matriz em `docs/CTO-NETWORK-DISTRIBUTION.md` §32), e os outros quatro são de `CTO-3`/`CTO-5`/`CTO-7`, que não existem em código. A única lacuna encontrada era de **cobertura, não de comportamento**: o invariante "o vínculo não pertence ao ciclo de vida da OS" era estruturalmente verdadeiro — único escritor, zero `delete` em produção, FK `SetNull` — e não tinha teste. Duas medições que não se redescobrem: removendo o pré-check de porta ocupada as **corridas continuam passando**, porque o índice parcial segura a integridade e o que cai é o `409` limpo do caso sequencial; e a sabotagem de TOCTOU é detectada sempre por `RACE-04` e só em metade das vezes por `RACE-02`, porque o caminho pré-lock do `MOVE` é mais longo. **A UI Web não tem tela de histórico de vínculo** — `getCustomerNetworkView.history` existe e não tem consumidor de tela; é decisão, não defeito.
