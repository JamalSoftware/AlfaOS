...
**Verificação automática de conectividade — `DIAG-AUTO-1`** (`docs/CTO-NETWORK-DISTRIBUTION.md` §48; PRD §370 e §390; `docs/SECURITY.md` §8.24). `npm run diagnostics:refresh` reconfere os clientes com conexão física ATIVA cuja última verificação venceu (~5 min). **É um comando, não um daemon** — o repositório não tem agendador, e o padrão é o de `scripts/outbox-worker.ts`; quem agenda é o operador, e sem isso nada se atualiza sozinho (a tela avisa com "Verificação atrasada"). **Duas colunas novas em `CustomerDiagnosticSnapshot`:** `statusSince` (desde quando o estado atual começou — NUNCA derivar duração de `observedAt`, que é reescrito a cada verificação) e `refreshLeaseUntil` (a reserva que impede dois ciclos de chamarem o mesmo cliente). **Não existe `STALE`**, não existe segunda autoridade de estado, e falha de provider continua sem escrever nada. O ciclo não passa pelo teto de 10/min, que é de usuário.
