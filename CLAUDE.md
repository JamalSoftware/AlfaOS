...
**Decisões do dono pendentes na `RC-1C`:** limite de **precisão** do GPS (não existe; a precisão é registrada, mostrada e não bloqueia) e o **backfill do legado** `RC-LOC-04` — 1 cliente real com projeção e sem autoridade, proposta pelo escritor automático existente (`applyImportedCustomerLocation`), **não executada** (`docs/MASTER-PLAN.md` §12).

**Próxima fase recomendada: `RC-1D`** — copy desatualizada, códigos crus da timeline da OS, acessibilidade e o E2E intermitente `MAPEDIT` (`docs/MASTER-PLAN.md` §12). Não iniciada. **Não iniciar `FIELD-MAP-1` durante a `RC-1` sem nova ordem do OWNER.**
