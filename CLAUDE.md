...
**`RC-1D` — UX, copy e observabilidade da CTO — `READY FOR OWNER VALIDATION` (15/09/2026). Commits locais, sem tag e sem push.** Os débitos de UX do `docs/MASTER-PLAN.md` §12 mais uma melhoria pequena que o dono aprovou na abertura: **ao abrir uma CTO, enxergar como estão os clientes dela**. **Zero migration, zero dependência, zero rota nova, zero Dart.**
...
**`RC-1D` — ADDENDUM de UX + `DIAG-AUTO-1` — `READY FOR OWNER VALIDATION` (15/09/2026). Commits locais, sem tag e sem push.** A validação do dono aprovou o conceito de conectividade na CTO e **reprovou a organização visual**, e apontou junto uma lacuna operacional real: *"Online · última leitura há 9 dias"* não prova que o cliente continua online. **Uma migration aditiva (duas colunas), zero dependência, zero rota nova, zero Dart.**
