...
**App Shell do Field — Fase 1 entregue, `main` local, sem tag e sem push.** O Field deixou de ter a Jornada escondida na terceira aba ("Mais") e ganhou a navegação híbrida que o PRD §255 já descrevia: barra principal com **Início, OS e Jornada**, gaveta global categorizada (`OPERACIONAL` e `CONTA` — só o que existe, nada de item `PLANNED` como placeholder, PRD §256), sino de notificações no cabeçalho, e o dashboard `Início` com os cards de Jornada (`allowedActions` do servidor, sem recalcular nada) e Ordens de Serviço (contagens reais, "Próxima OS" só quando alguma tem `scheduledAt`, sem inventar ordenação). **Mapa não é destino da barra nesta fase** — o próprio §255 previa isso ("fica vago ou traz Agenda" enquanto o módulo não existe), e nem Mapa nem Agenda têm código ainda.

Achado corrigido durante a implementação, não só relatado: a gaveta nasceu presa ao `Scaffold` de cada tela e colidia com a barra inferior — corrigido dando a ela um único dono (`Scaffold` do shell), com cada tela abrindo essa mesma instância por referência. Auditoria focal (não clean-room, compensada com ataques novos): `APPROVED WITH RISKS`, 0 CRITICAL/HIGH/MEDIUM. Nenhuma migration; nenhum arquivo web/backend tocado. Gates verdes: 1423 Vitest e 99 Playwright inalterados, 247 Flutter (+22), APK debug construído. **Aguarda piloto em aparelho físico antes de tag e push** — PRD §255, §257.

**Depois do piloto: implementar a Escala de Trabalho P0 (PRD §307)** — escala básica, plantão, folga, DSR, sábado alternado, calendário web, "Minha Escala" no Field —, começando por decidir schema e migration antes de qualquer rota.
