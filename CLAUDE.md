...
**Próxima fase recomendada: `RC-1D`** — copy desatualizada, códigos crus da timeline da OS, acessibilidade e o E2E intermitente `MAPEDIT` (`docs/MASTER-PLAN.md` §12) —, **depois** da validação física da `RC-1C-HOTFIX`, que é o que fecha a `RC-1C`. Não iniciada. **Não iniciar `FIELD-MAP-1` durante a `RC-1` sem nova ordem do OWNER.**
