...
**Risco de dependência, medido:** `npm audit` reporta 14 vulnerabilidades; **8 `high` são pré-existentes** (Next, Prisma, ESLint) e as **6 `moderate` chegaram com o `firebase-admin`**, na cadeia `@google-cloud/storage`. Medido: importar `firebase-admin/app` e `firebase-admin/messaging` carrega 98 módulos e **nenhum** deles é dessa cadeia. Nenhum upgrade foi feito.
...
**`NF-5` — piloto em aparelho físico — continua `PLANNED`, e a §27.5 é o argumento de que ele é obrigatório:** gesto de sistema, toque em notificação real e ordem de subida não se aprovam por teste de widget. Depende do `google-services.json`, que é pendência do operador. Os commits da NF-1 a NF-4 são locais, não publicados, e **não devem ser reescritos, amendados nem squashados**.
...
**`SGP-1` ENTREGUE — commits locais, sem tag e sem push. `SGP SANDBOX VALIDATION REQUIRED`.** O provider SGP existe no domínio, autentica e é ativável — **e nada além disso**. Migration **aditiva de duas linhas** (`20260903120000`: `ERPProvider.SGP` e `ERPCredentialKind.PUBLIC_API`), zero coluna, zero tabela, zero unique alterada, zero dependência, zero Dart. Registro em `docs/ERP-INTEGRATIONS.md` §29.
