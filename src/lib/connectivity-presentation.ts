 ...
 * ponto de uso: quem quiser escrever a duração é obrigado a passar
 * `statusSince`, e passar `observedAt` no lugar dele produz uma frase que os
 * testes derrubam. Era essa troca — silenciosa, de um campo pelo outro — que
 * faria a verificação de 5 em 5 minutos reiniciar toda duração da tela.
 *
 * `null` quando nunca houve observação: sem leitura não há estado a durar.
 */
export function connectivityStatusDuration(
  statusSince: string | null,
  now: Date = new Date(),
): string | null {
  return connectivityAge(statusSince, now);
}
