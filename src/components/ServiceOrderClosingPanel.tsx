...
export function ServiceOrderClosingPanel({
  ...
  return (
    <div className="space-y-4">
      ...
      {notice && !error && (
        <div className="rounded-lg border border-success-border bg-success-bg px-3 py-2 text-sm text-success-fg">
          {notice}
        </div>
      )}
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
        ...
        <input
          ...
          className="block w-full text-sm text-fg-secondary file:mr-3 file:rounded-lg file:border-0 file:bg-primary file:px-4 file:py-2.5 file:text-sm file:font-semibold file:text-primary-fg disabled:opacity-50"
        />
        {busy === "evidence" && (
          <p className="mt-2 text-sm text-fg-muted">Enviando foto...</p>
        )}
        {evidences.length > 0 ? (
            ...
            {evidences.map((ev) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={disabled}
                  className="w-full px-2 py-2 text-xs font-medium text-danger-fg disabled:opacity-50"
                >
                  Remover
                </button>
              </li>
          ...
          <p className="mt-3 text-sm text-fg-muted">Nenhuma foto anexada.</p>
        )}
      </section>
        ...
        <form onSubmit={addMaterial} className="space-y-3">
          <div>
            ...
            <input
              ...
              disabled={disabled}
              className="w-full rounded-lg border border-input-border px-3 py-2.5 text-base text-fg focus:border-focus focus:outline-none"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={disabled}
            className="w-full rounded-lg border border-input-border px-3 py-2.5 text-sm font-semibold text-fg-secondary disabled:opacity-50"
          >
            ...
            {materials.map((m) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={disabled}
                  className="shrink-0 text-xs font-medium text-danger-fg disabled:opacity-50"
                >
                  Remover
                </button>
              </li>
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
        ...
        {signature && (
          <p className="mb-3 rounded-lg bg-success-bg px-3 py-2 text-sm text-success-fg">
      ...
      <section className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
        ...
        <dl className="mb-4 space-y-1 text-sm">
          <div className="flex justify-between">
            <dt className="text-fg-muted">Fotos</dt>
            <dd className="font-medium text-fg">{evidences.length}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-fg-muted">Materiais</dt>
            <dd className="font-medium text-fg">{materials.length}</dd>
          </div>
          <div className="flex justify-between">
            <dt className="text-fg-muted">Assinatura</dt>
        ...
        {!canComplete && (
          <p className="mb-3 rounded-lg border border-warning-border bg-warning-bg px-3 py-2 text-sm text-warning-fg">
        ...
        <button
          ...
          className="w-full rounded-xl bg-confirm px-4 py-4 text-base font-bold uppercase tracking-wide text-primary-fg shadow-sm transition-colors hover:bg-confirm-hover disabled:cursor-not-allowed disabled:opacity-60"
        >
